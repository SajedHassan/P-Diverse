...
def create_types_paths_fn(folder, files, file_ending, f):
    p = re.compile(re.escape(f) + r"_type" + re.escape(file_ending))            
    return [join(folder, i) for i in files if p.fullmatch(i)]
...
def create_types_from_splitted_dataset_folder(folder: str, file_ending: str, identifiers: List[str] = None, num_processes: int = 12) -> List[
    List[str]]:
    """
    does not rely on dataset.json
    """
    if identifiers is None:
        identifiers = get_identifiers_from_splitted_dataset_folder(folder, file_ending)
    files = subfiles(folder, suffix=('_type' + file_ending), join=False, sort=True)
    list_of_lists = []

    params_list = [(folder, files, file_ending, f) for f in identifiers]
    with Pool(processes=num_processes) as pool:
        list_of_lists = pool.starmap(create_types_paths_fn, params_list)
...
def get_filenames_of_train_images_and_targets(raw_dataset_folder: str, dataset_json: dict = None):
    ...
    if 'dataset' in dataset_json.keys():
        ...
        types = [join(raw_dataset_folder, 'imagesTr', i + '_type.txt') for i in identifiers]
        dataset = {i: {'images': im, 'label': se, 'type': t} for i, im, se, t in zip(identifiers, images, segs, types)}
    return dataset
